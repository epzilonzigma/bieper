...
- **Close processes and ports you open.** Any process you start that binds a port or runs in the background — `yarn dev`, a Playwright / browser-MCP session, a preview server — must be shut down once the task it was started for completes, even on failure or interruption. Confirm nothing you launched is still listening (e.g. `lsof -i :3000`). Only stop what you started; leave pre-existing servers running. This applies to every agent and subagent — see `AGENTS.md` → "Clean up processes and ports".
