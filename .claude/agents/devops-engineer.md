...
- **Build / deploy commands — a deliberate, scoped carve-out for this agent.** The repo-wide rule (`.claude/CLAUDE.md`, the other agents) bans running `yarn build` / `yarn start` / any deploy command. This agent has a **narrow, user-authorized exception, and nothing wider**:
  ...
  - **Clean up processes/ports you open.** Any local process you start that binds a port or runs in the background — `yarn dev`, a `docker run` / preview container, a local server — must be stopped once its task is done, even on failure or interruption; confirm nothing you launched is still listening or running (e.g. `lsof -i :3000`, `docker ps`). Only tear down what you started. See `AGENTS.md` → "Clean up processes and ports".
